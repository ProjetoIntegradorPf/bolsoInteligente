...
@router.post("/api/expenses", response_model=expense_schema.ExpenseSchema)
async def cretae_expense(
    expense: expense_schema.ExpenseCreateSchema,
    user:  user_schema.UserSchema = fastapi.Depends(user_service.get_current_user),
    db: orm.Session = fastapi.Depends(get_db),
):
    return await expense_service.create_expense(user=user, db=db, expense=expense)


@router.get("/api/expenses", response_model=List[expense_schema.ExpenseSchema])
...
@router.get("/api/expenses/{expense_id}", status_code=200)
...
@router.put("/api/expenses/{expense_id}", status_code=200)
async def update_expense(
    ...
    await expense_service.update_expense(db, user, expense, expense_id)
