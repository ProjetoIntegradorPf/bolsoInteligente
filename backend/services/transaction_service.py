...
async def check_transaction_categories(
        db: Session,
        user: user_schema.UserSchema,
        transaction: transaction_schema.TransactionCreateSchema):
    if transaction.category_expense_id is not None:
        if transaction.category_revenue_id is not None or transaction.category_investment_id is not None:
            raise HTTPException(
                status_code=422,
                detail="Cannot set category_revenue_id or category_investment_id when category_expense_id is set")
        expense = await get_expense_by_id(db, user, transaction.category_expense_id)
        if not expense:
            raise HTTPException(
                status_code=404,
                detail="Category Expense not found")
        return expense
    if transaction.category_revenue_id is not None:
        if transaction.category_expense_id is not None or transaction.category_investment_id is not None:
            raise HTTPException(
                status_code=422,
                detail="Cannot set category_expense_id or category_investment_id when category_revenue_id is set")
        revenue = await get_revenue_by_id(db, user, transaction.category_revenue_id)
        if not revenue:
            raise HTTPException(
                status_code=404,
                detail="Category Revenue not found")
        return revenue
    if transaction.category_investment_id is not None:
        if transaction.category_expense_id is not None or transaction.category_revenue_id is not None:
            raise HTTPException(
                status_code=422,
                detail="Cannot set category_expense_id or category_revenue_id when category_investment_id is set")
        investment = await get_investment_by_id(db, user, transaction.category_investment_id)
        if not investment:
            raise HTTPException(
                status_code=404,
                detail="Category Investment not found")
        return investment
