...
async def get_expense_by_id(db: Session, user: user_schema.UserSchema, expense_id: int):
    ...
    if expense is None:
        raise fastapi.HTTPException(status_code=404, detail="Expense not found")
...
async def delete_expense(db: Session, user: user_schema.UserSchema, expense_id: int):
    ...
    if transactions:
        raise fastapi.HTTPException(status_code=400, detail="This expense is in use and cannot be deleted")
