...
import schemas.investment_schema as investment_schema, schemas.user_schema as user_schema
...
async def get_investment_by_id(db: Session, user: user_schema.UserSchema, investment_id: int):
    ...
    if investment is None:
        raise fastapi.HTTPException(status_code=404, detail="Investment not found")
...
async def delete_investment(db: Session, user: user_schema.UserSchema, investment_id: int):
    ...
    if transactions:
        raise fastapi.HTTPException(status_code=400, detail="This investment is in use and cannot be deleted")
