...
import schemas.revenue_schema as revenue_schema, schemas.user_schema as user_schema
...
async def delete_revenue(db: Session, user: user_schema.UserSchema, revenue_id: int):
    ...
    if transactions:
        raise HTTPException(status_code=400, detail="This revenue is in use and cannot be deleted")
